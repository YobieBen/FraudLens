        ...
        try:
            ...
            for message in messages:
                result = await self.process_email(
                    message["id"], process_attachments=process_attachments
                )
    ...
    async def process_email(
        ...
        try:
            ...
            return EmailAnalysisResult(
                ...
                explanation=(
                    content_result.explanation if content_result else "No analysis available"
                ),
    ...
    async def _process_attachment(self, attachment: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            att_data = (
                ...
                .attachments()
                .get(
                    userId="me", messageId=attachment["message_id"], id=attachment["attachment_id"]
                )
                .execute()
            )
            ...
            if result:
                return {
                    ...
                    "fraud_types": (
                        [str(ft) for ft in result.fraud_types] if result.fraud_types else []
                    ),
    ...
    async def _label_email(self, message_id: str, fraud_score: float):
        ...
        try:
            ...
            self.service.users().messages().modify(
                userId="me", id=message_id, body={"addLabelIds": [l for l in labels_to_add if l]}
            ).execute()
    ...
    async def _take_action(self, result: EmailAnalysisResult):
        ...
        try:
            if result.action_taken == EmailAction.TRASH:
                # Move to trash
                self.service.users().messages().trash(userId="me", id=result.message_id).execute()
                logger.info(f"Moved email {result.message_id} to trash")

            elif result.action_taken == EmailAction.SPAM:
                # Move to spam
                self.service.users().messages().modify(
                    userId="me", id=result.message_id, body={"addLabelIds": ["SPAM"]}
                ).execute()
                ...
                if quarantine_label:
                    self.service.users().messages().modify(
                        ...
                        body={"addLabelIds": [quarantine_label], "removeLabelIds": ["INBOX"]},
                    ).execute()
                    logger.info(f"Quarantined email {result.message_id}")

            # Update stats
            action_name = result.action_taken.value
            self.stats["actions_taken"][action_name] = (
                self.stats["actions_taken"].get(action_name, 0) + 1
            )
...
async def main():
    ...
    results = await scanner.stream_emails(
        query="is:unread", max_results=10, process_attachments=True
    )
