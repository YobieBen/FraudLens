...
from fraudlens.api.auth import (
    auth_manager, UserCreate, UserInDB, Token, TokenData,
    UserRole, create_tokens, verify_token, authenticate_user,
    create_api_key, verify_api_key, check_rate_limit
)
...
async def get_current_user_from_api_key(api_key: Optional[str] = Depends(api_key_header)) -> Optional[Dict]:
