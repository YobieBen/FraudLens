        ...
        for row in cursor.fetchall():
            data_records.append(
                PersonalData(
                    ...
                    consent_timestamp=(
                        datetime.fromisoformat(row["consent_timestamp"])
        ...
        if row:
            return ConsentRecord(
                ...
                withdrawal_timestamp=(
                    datetime.fromisoformat(row["withdrawal_timestamp"])
                    if row["withdrawal_timestamp"]
                    else None
                ),
            )
...
class ComplianceManager:
    ...
    async def anonymize_data(
        self, data: Dict[str, Any], fields: List[str], method: str = "redact"
    ) -> Dict[str, Any]:
        """
    ...
    async def delete_user_data(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"user_id": user_id, "records_deleted": records_deleted, "services": services}
    ...
    def record_consent(
        ...
        self.log_audit(
            ...
            result="success",
            user_id=user_id,
            metadata={"purpose": purpose, "granted": granted, "version": version},
        )
    ...
    def withdraw_consent(self, user_id: str, purpose: str, ip_address: Optional[str] = None):
        """Withdraw user consent"""

        self.record_consent(user_id=user_id, purpose=purpose, granted=False, ip_address=ip_address)
    ...
    async def get_audit_log(
        ...
        entries = self.db.get_audit_log(
            start_date=start_date, end_date=end_date, user_id=user_id, limit=limit * page
        )
    ...
    def log_deletion(self, user_id: str, records_deleted: int, timestamp: datetime):
        ...
        self.log_audit(
            ...
            result="success",
            user_id=user_id,
            metadata={"records_deleted": records_deleted, "timestamp": timestamp.isoformat()},
        )
        ...
        if standard == ComplianceStandard.GDPR:
            ...
            report["metrics"] = {
                ...
                "deletion_requests": len(
                    [l for l in audit_logs if l.action == "user_data_deletion"]
                ),
    ...
    def _convert_to_csv(self, data: Dict[str, Any]) -> str:
        ...
        if "personal_data" in data:
            ...
            for item in data["personal_data"]:
                output.append(
                    f"{item['type']},{item['value']},{item['collected_at']},{item['purpose']}"
                )
            output.append("")
        ...
        if "audit_logs" in data:
            ...
            for log in data["audit_logs"]:
                output.append(
                    f"{log['timestamp']},{log['action']},{log['resource']},{log['result']}"
                )
    ...
    def is_healthy(self) -> bool:
        ...
        try:
            ...
            self.db.get_audit_log(start_date=datetime.utcnow(), end_date=datetime.utcnow(), limit=1)
            return True
        except:
            return False
