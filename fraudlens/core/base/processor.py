...
class ProcessedData:
    ...
    def get_tensor(self, framework: str = "numpy") -> Any:
        ...
        if framework == "numpy":
            ...
            try:
                import torch
                return torch.from_numpy(self.data) if isinstance(self.data, np.ndarray) else self.data
            except ImportError:
...
class ModalityProcessor(ABC):
    ...
    async def batch_process(
        self,
        inputs: List[Union[str, bytes, np.ndarray, Path]],
        batch_size: int = 32,
        **kwargs
    ) -> List[ProcessedData]:
        """
    ...
    async def _process_batch(
        self,
        batch: List[Union[str, bytes, np.ndarray, Path]],
        **kwargs
    ) -> List[ProcessedData]:
