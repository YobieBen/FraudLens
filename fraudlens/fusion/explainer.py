    ...
    def _create_audit_trail(
        ...
        for modality, score in fused_result.modality_scores.items():
            audit_trail.append({
                ...
                "details": {
                    "score": score,
                    "processing_time_ms": fused_result.processing_time_ms / len(fused_result.modality_scores),
                },
            })
        ...
        if timestamp:
            ...
            timestamp_str = metadata.get("timestamp", datetime.now()).isoformat() if metadata else datetime.now().isoformat()
        
        # Get risk level and overall risk score with fallback
        risk_level = getattr(risk_score, 'risk_level', 'unknown')
        overall_risk = getattr(risk_score, 'overall_risk', None) or getattr(risk_score, 'risk_score', 0.0)
        ...
        if consistency_report:
            audit_trail.append({
                ...
                "details": {
                    ...
                    "high_risk_inconsistencies": len(consistency_report.high_risk_inconsistencies),
                },
            })
...
class ReportExporter:
    ...
    async def _export_json(
        ...
        # Handle both RiskProfile and RiskScore objects
        overall_risk = getattr(risk_score, 'overall_risk', None) or getattr(risk_score, 'risk_score', 0.0)
        confidence_interval = getattr(risk_score, 'confidence_intervals', None) or getattr(risk_score, 'confidence_interval', (0.0, 1.0))
