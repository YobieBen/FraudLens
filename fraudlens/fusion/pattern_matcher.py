...
class Pattern:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Pattern":
        ...
        return cls(
            ...
            last_matched=(
                datetime.fromisoformat(data["last_matched"]) if data.get("last_matched") else None
            ),
...
class PatternLibrary:
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        for pattern in self.patterns.values():
            ...
            stats["patterns_by_severity"][pattern.severity] = (
                stats["patterns_by_severity"].get(pattern.severity, 0) + 1
            )

        # Find most matched patterns
        sorted_by_matches = sorted(
            self.patterns.values(), key=lambda p: p.match_count, reverse=True
        )
        stats["most_matched"] = [
        ...
        high_fp = [
            p for p in self.patterns.values() if p.false_positive_count > p.match_count * 0.3
        ]
        stats["high_false_positive"] = [
            {
                "id": p.pattern_id,
                "name": p.name,
                "fp_rate": p.false_positive_count / max(p.match_count, 1),
            }
            for p in high_fp
        ]
    ...
    def _match_behavioral(self, pattern: Pattern, data: Dict[str, Any]) -> Optional[Match]:
        ...
        if behavior_type == "rapid_transfers":
            ...
            for i in range(len(transactions) - 1):
                time_diff = transactions[i + 1].get("timestamp", 0) - transactions[i].get(
                    "timestamp", 0
                )
        ...
        for entity in entities:
            ...
            if len(connections) >= min_connections:
                ...
                for attr in shared_attributes:
                    ...
                    if (
                        values and len(set(values)) < len(values) * 0.5
                    ):  # More than half share value
                        shared_count += 1
