    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "text_image_consistency": (
                self.text_image_consistency.to_dict() if self.text_image_consistency else None
            ),
            "audio_video_sync": self.audio_video_sync.to_dict() if self.audio_video_sync else None,
            "metadata_content_validation": (
                self.metadata_content_validation.to_dict()
                if self.metadata_content_validation
                else None
            ),
            "entity_consistency": (
                self.entity_consistency.to_dict() if self.entity_consistency else None
            ),
            "temporal_consistency": (
                self.temporal_consistency.to_dict() if self.temporal_consistency else None
            ),
...
class CrossModalValidator:
    ...
    async def validate_consistency(
        ...
        if text_result and image_result:
            validation_tasks.append(self._validate_text_image(text_result, image_result, raw_data))
        ...
        if raw_data and raw_data.get("metadata"):
            validation_tasks.append(self._validate_metadata_content(raw_data))

        # Entity consistency
        results = [r for r in [text_result, image_result, audio_result, video_result] if r]
        if len(results) > 1:
            validation_tasks.append(self._validate_entity_consistency(results, raw_data))
        ...
        # Text-image consistency
        if text_result and image_result:
            if result_idx < len(validation_results) and not isinstance(
                validation_results[result_idx], Exception
            ):
        ...
        for result in validation_results:
            ...
            if result:
                consistency_scores.append(
                    result.confidence if result.is_valid else 1 - result.confidence
                )
    ...
    async def _validate_metadata_content(
        ...
        if metadata_author and content_author:
            if metadata_author.lower() != content_author.lower():
                inconsistencies.append(
                    f"Author mismatch: metadata={metadata_author}, content={content_author}"
                )
        ...
        if metadata_format and detected_format:
            if metadata_format != detected_format:
                inconsistencies.append(
                    f"Format mismatch: metadata={metadata_format}, detected={detected_format}"
                )
            ...
            if union:
                ...
                for i, entities1 in enumerate(all_entities):
                    for j, entities2 in enumerate(all_entities[i + 1 :], i + 1):
                        ...
                        if conflicts:
                            inconsistencies.append(
                                f"Conflicting entities between modalities {i} and {j}: {conflicts}"
                            )
    ...
    async def _validate_temporal_consistency(
            ...
            if content_dates and metadata_dates:
                for c_date in content_dates:
                    for m_date in metadata_dates:
                        if c_date > m_date:
                            inconsistencies.append(
                                f"Content date {c_date} after metadata date {m_date}"
                            )
            ...
            if events:
                ...
                for i in range(len(sorted_events) - 1):
                    ...
                    if not self._validate_event_sequence(event1, event2):
                        inconsistencies.append(
                            f"Invalid event sequence: {event1['type']} -> {event2['type']}"
                        )

        # Check processing time consistency
        processing_times = [
            r.processing_time_ms for r in results if hasattr(r, "processing_time_ms")
        ]
        if processing_times:
        ...
        evidence["timestamps"] = [
            ts.isoformat() if hasattr(ts, "isoformat") else str(ts) for ts in timestamps
        ]
