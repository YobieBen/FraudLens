...
class MonitoringDashboard:
    ...
    def collect_metrics(self) -> Dict[str, Any]:
        ...
        if fraud_metrics:
            self.metrics_history["latency"].append(
                fraud_metrics.get("avg_latency_ms", 0)
            )
            self.metrics_history["throughput"].append(
                fraud_metrics.get("requests_per_second", 0)
            )
            self.metrics_history["error_rate"].append(
                fraud_metrics.get("error_rate", 0)
            )
    ...
    def create_fraud_metrics_plot(self) -> go.Figure:
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=("Fraud Score Distribution", "Detection Accuracy",
                          "False Positive Rate", "Processing Time"),
        ...
        fig.update_layout(
            height=600,
            showlegend=False,
            title_text="Fraud Detection Metrics"
        )
                ...
                with gr.Column(scale=2):
                    with gr.Tab("Performance"):
                        perf_plot = gr.Plot(
                            ...
                            value=self.create_performance_plot()
                        )
                    
                    with gr.Tab("Fraud Metrics"):
                        fraud_plot = gr.Plot(
                            label="Fraud Detection Metrics",
                            value=self.create_fraud_metrics_plot()
                        )
                    ...
                    with gr.Tab("Cost Tracking"):
                        cost_plot = gr.Plot(
                            label="Cost Analysis",
                            value=self.create_cost_tracking_plot()
                        )
            ...
            with gr.Row():
                with gr.Column():
                    ...
                    recommendations_df = gr.Dataframe(
                        value=pd.DataFrame(self.create_optimization_recommendations()),
                        height=200
                    )
            ...
            def export_report():
                ...
                report = {
                    ...
                    "metrics_summary": {
                        ...
                        "avg_throughput_rps": np.mean(list(self.metrics_history["throughput"]) or [0]),
                        "avg_cpu_percent": np.mean(list(self.metrics_history["cpu_usage"]) or [0]),
                        "avg_memory_percent": np.mean(list(self.metrics_history["memory_usage"]) or [0])
                    }
                }
