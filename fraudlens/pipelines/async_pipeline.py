...
class AsyncPipeline:
    ...
    async def process(
        ...
        # Handle batch input
        tasks = [
            self._create_task(data, modality, priority)
        ...
        try:
            ...
            if task.modality in self._processors:
                try:
                    ...
                    processed_data = await asyncio.wait_for(
                        processor.process(task.input_data),
            ...
            if processed_data or task.modality not in self._processors:
                ...
                for detector_id, detector in self._detectors.items():
                    if detector.modality == task.modality or detector.modality == Modality.MULTIMODAL:
                        detection_tasks.append(
                            self._run_detector(detector, input_for_detection)
                        )
            ...
            if detection_results and self._scorers:
                try:
                    ...
                    risk_assessment = await asyncio.wait_for(
                        scorer.score(detection_results),
                        timeout=self.timeout_seconds / 3
                    )
