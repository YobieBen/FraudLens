...
class PluginManager:
    ...
    async def _load_plugin_file(self, plugin_file: Path):
        ...
        if spec and spec.loader:
            ...
            for name, obj in inspect.getmembers(module):
                if (
                    inspect.isclass(obj)
                    and issubclass(obj, FraudDetectorPlugin)
                    and obj != FraudDetectorPlugin
                ):
    ...
    async def execute_plugin(self, plugin_name: str, input_data: Any, **kwargs) -> Optional[Dict]:
        """
    ...
    async def execute_plugins(
        self, input_data: Any, detection_result: Any = None, **kwargs
    ) -> List[Dict]:
        """
