...
class FraudResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "social_eng_result": (
                self.social_eng_result.to_dict() if self.social_eng_result else None
            ),
...
class TextFraudDetector(FraudDetector):
    ...
    async def initialize(self) -> None:
        ...
        self.llm_manager = LLMManager(device=self.device, model_path=self.model_path)
        await self.llm_manager.initialize()

        # Initialize cache manager
        self.cache_manager = CacheManager(
            max_size=self.config["cache_size"], ttl_seconds=3600  # 1 hour TTL
        )
        ...
        self.social_eng_analyzer = SocialEngineeringAnalyzer(
            self.llm_manager, self.feature_extractor
        )
        self.financial_doc_analyzer = FinancialDocumentAnalyzer(
            self.llm_manager, self.feature_extractor
        )
        self.money_laundering_analyzer = MoneyLaunderingAnalyzer(
            self.llm_manager, self.feature_extractor
        )
    ...
    async def _analyze_text(self, text: str) -> FraudResult:
        ...
        if (
            features.get("has_urls")
            or features.get("has_email_patterns")
            or has_suspicious
            or features.get("urgency_score", 0) > 0.3
        ):
        ...
        is_financial_doc = any(
            term in text_lower for term in ["invoice", "receipt", "statement", "contract"]
        )
        ...
        if features.get("has_transaction_patterns"):
            analysis_tasks.append(
                ("money_laundering", self.money_laundering_analyzer.analyze(text))
            )
    ...
    def _create_detection_result(
        ...
        fraud_types = []
        text_lower = str(
            fraud_result.get("text", "")
            if isinstance(fraud_result, dict)
            else getattr(fraud_result, "text", "")
        ).lower()
        ...
        if any(
            word in text_lower
            for word in ["ssn", "social security", "identity", "maiden name", "date of birth"]
        ):
        ...
        if any(
            word in text_lower for word in ["account takeover", "unauthorized access", "hijacked"]
        ):
    ...
    async def detect_video_fraud(
        self, video_path: str, sample_rate: int = 10, max_frames: int = 300
    ) -> VideoAnalysisResult:
        """
        ...
        return await self.video_fraud_detector.analyze_video(video_path, sample_rate, max_frames)
    ...
    async def detect_image_deepfake(
        ...
        return await self.deepfake_detector.detect_image_deepfake(image_path, return_visualization)
