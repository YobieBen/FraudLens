...
class DocumentForgeryDetector:
    ...
    async def detect(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        is_forged = len(indicators) > 0 and max(confidence_scores, default=0) > (1.0 - self.sensitivity)
    ...
    def _has_unnatural_edges(self, image: np.ndarray) -> bool:
        ...
        if len(edge_points[0]) > 100:
            ...
            sample_idx = np.random.choice(len(edge_points[0]), min(100, len(edge_points[0])), replace=False)
    ...
    def _has_scanning_artifacts(self, image: np.ndarray) -> bool:
        ...
        distances = np.sqrt((np.arange(h)[:,None] - center_h)**2 + 
                          (np.arange(w) - center_w)**2)
    ...
    def _analyze_signatures(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            ...
            "authentic": all(
                self._check_ink_consistency(s) and 
                self._check_stroke_patterns(s) and
                self._check_pressure_variations(s)
                for s in signatures
    ...
    def _analyze_stamps(self, image: np.ndarray) -> Dict[str, Any]:
        ...
        return {
            "count": len(stamps),
            "authentic": all(
                self._check_stamp_clarity(s) and
                self._check_ink_bleeding(s) and
                self._check_stamp_alignment(s)
                for s in stamps
            ) if stamps else True,
        }
