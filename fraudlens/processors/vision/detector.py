...
class VisionFraudDetector(FraudDetector):
    ...
    def __init__(
        ...
        logger.info(
            f"VisionFraudDetector initialized (GPU: {self.enable_gpu}, Metal: {self.use_metal})"
        )
    ...
    async def detect(
        self, input_data: Union[str, Path, bytes, np.ndarray], **kwargs
    ) -> DetectionResult:
        """
    ...
    async def process_image(self, image_path: Union[str, Path]) -> ImageAnalysis:
        ...
        if self._is_photo(image):
            analysis_tasks.append(("deepfake", self.deepfake_detector.detect(processed_image)))

        # Manipulation detection
        analysis_tasks.append(("manipulation", self.manipulation_detector.analyze(image_path)))

        # Document forgery for document images
        if self._is_document(image):
            analysis_tasks.append(("forgery", self.forgery_detector.detect(processed_image)))

            # OCR for text extraction
            analysis_tasks.append(("ocr", self.ocr_engine.extract_text(processed_image)))

        # Logo detection
        analysis_tasks.append(("logo", self.logo_detector.detect_logos(processed_image)))

        # QR code analysis
        analysis_tasks.append(("qr", self.qr_analyzer.analyze(processed_image)))

        # Feature extraction
        analysis_tasks.append(("features", self.feature_extractor.extract(processed_image)))
        ...
        extracted_text = (
            results.get("ocr", {}).get("text", None) if extracted_text is None else extracted_text
        )
    ...
    async def process_pdf(self, pdf_path: Union[str, Path]) -> DocumentAnalysis:
        ...
        inconsistencies = self._check_document_consistency(
            page_analyses, pdf_data.get("metadata", {}), tables, entities
        )
        ...
        authenticity_score = self._calculate_authenticity_score(
            page_analyses, pdf_data.get("metadata", {}), inconsistencies
        )
    ...
    async def batch_process(
        self, inputs: List[Union[str, Path, bytes]], **kwargs
    ) -> List[ImageAnalysis]:
        """
    ...
    def _classify_document_type(self, text: str, entities: Dict) -> str:
        ...
        if any(term in text_lower for term in ["invoice", "bill to", "ship to", "subtotal"]):
            ...
            term in text_lower for term in ["driver", "license", "identification", "passport"]
        ):
            return "id_document"
    ...
    def _calculate_authenticity_score(
        self, page_analyses: List[ImageAnalysis], metadata: Dict, inconsistencies: List[str]
    ) -> float:
    ...
    def _convert_image_to_detection(self, analysis: ImageAnalysis) -> DetectionResult:
        ...
        # Create explanation
        explanation = (
            f"Image analysis detected: {', '.join(analysis.fraud_types)}"
            if analysis.fraud_types
