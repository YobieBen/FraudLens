...
class PDFProcessor:
    ...
    def __init__(
        ...
        logger.info(
            f"PDFProcessor initialized (PyMuPDF: {self.has_pymupdf}, pdf2image: {self.has_pdf2image})"
        )
    ...
    async def _process_with_pymupdf(
        self, pdf_path: Path, extract_text: bool, extract_images: bool
    ) -> Dict[str, Any]:
    ...
    async def _process_with_pdf2image(self, pdf_path: Path, extract_text: bool) -> Dict[str, Any]:
    ...
    async def _mock_process(self, pdf_path: Path) -> Dict[str, Any]:
        ...
        mock_pages = [
            np.ones((self.dpi * 11, self.dpi * 8, 3), dtype=np.uint8) * 255 for _ in range(3)
        ]
    ...
    async def progressive_load(
        self, pdf_path: Union[str, Path], callback: callable, chunk_size: int = 10
    ) -> None:
        """
        Progressive loading for large PDFs.
    ...
    async def extract_page_range(
        self, pdf_path: Union[str, Path], start_page: int, end_page: int
    ) -> List[np.ndarray]:
