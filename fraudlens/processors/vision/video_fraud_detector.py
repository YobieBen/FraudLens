...
class VideoFraudDetector:
    ...
    async def analyze_video(
        ...
        while cap.isOpened() and analyzed_count < max_frames:
            ...
            if frame_count % sample_rate == 0:
                # Analyze frame
                frame_result = await self._analyze_frame(frame, frame_count, prev_gray)
        ...
        temporal_consistency = self._analyze_temporal_consistency(
            face_landmarks_history, optical_flow_scores
        )

        # Facial landmarks score
        facial_landmarks_score = self._analyze_facial_landmarks(face_landmarks_history)
        ...
        is_fraudulent = (
            deepfake_probability > self.deepfake_threshold
            or temporal_consistency < self.temporal_threshold
            or len(manipulation_regions) > 2
        )
    ...
    async def _analyze_frame(
        self, frame: np.ndarray, frame_number: int, prev_gray: Optional[np.ndarray] = None
    ) -> Dict[str, Any]:
        ...
        if not self.face_mesh:
            ...
            if len(faces) > 0:
                ...
                result["face_landmarks"] = np.array(
        ...
        if manipulation_mask is not None:
            contours, _ = cv2.findContours(
                manipulation_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
            ...
            if contours:
                ...
                if area > 1000:  # Significant manipulation area
                    ...
                    result["manipulation_confidence"] = min(
                        area / (frame.shape[0] * frame.shape[1]), 1.0
                    )
                    result["is_suspicious"] = True
    ...
    def _analyze_optical_flow(self, prev_gray: np.ndarray, curr_gray: np.ndarray) -> float:
    ...
    def _analyze_temporal_consistency(
        self, face_landmarks_history: List[np.ndarray], optical_flow_scores: List[float]
    ) -> float:
    ...
    def _analyze_facial_landmarks(self, face_landmarks_history: List[np.ndarray]) -> float:
                ...
                for i in range(0, h - block_size, block_size):
                    for j in range(0, w - block_size, block_size):
                        ...
                        if i > 0:
                            top_edge = np.mean(
                                np.abs(
                                    gray[i - 1, j : j + block_size] - gray[i, j : j + block_size]
                                )
                            )
    ...
    def _generate_explanation(
        ...
        if VideoFraudType.MANIPULATION in fraud_types:
            explanation_parts.append(f"• {len(manipulation_regions)} manipulated regions detected")

        if VideoFraudType.COMPRESSION_ARTIFACT in fraud_types:
            explanation_parts.append("• Compression artifacts suggest potential editing")
