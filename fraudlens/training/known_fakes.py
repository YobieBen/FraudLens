...
class KnownFakeDatabase:
    ...
    def check_document(self, image: np.ndarray, text_content: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        if text_content:
            ...
            for fake_id, pattern in self.fake_patterns.items():
                ...
                if match_ratio > 0.5:  # More than half the identifiers match
                    result["is_known_fake"] = True
                    result["confidence"] = max(result["confidence"], pattern["confidence"] * match_ratio)
