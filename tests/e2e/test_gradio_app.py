...
class GradioE2ETester:
    ...
    async def test_text_analysis(self):
            ...
            try:
                ...
                if "data" in result:
                    ...
                    if test_case["expected_high_fraud"]:
                        if "high" in fraud_score_text.lower() or "fraud" in fraud_score_text.lower():
                            logger.info(f"Test case {i+1} passed: High fraud detected as expected")
                        else:
                            logger.warning(f"Test case {i+1}: Expected high fraud but got: {fraud_score_text}")
                            all_passed = False
                    else:
                        if "low" in fraud_score_text.lower() or "legitimate" in fraud_score_text.lower():
                            logger.info(f"Test case {i+1} passed: Low fraud detected as expected")
                        else:
                            logger.warning(f"Test case {i+1}: Expected low fraud but got: {fraud_score_text}")
                else:
        ...
        self.log_result(test_name, all_passed, "All text analysis tests passed" if all_passed else "Some tests failed")
        return all_passed
