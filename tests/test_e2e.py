...
class TestScorer(RiskScorer):
    ...
    async def score(self, detection_results: List[DetectionResult], **kwargs) -> RiskAssessment:
        ...
        factors = [
            ...
            for r in detection_results if r.fraud_score > 0
        ]
...
class TestE2EFraudLens:
    ...
    async def test_04_risk_scorer(self):
        ...
        print(f"✓ RiskScorer tested - Risk: {assessment.risk_level.value}, Score: {assessment.overall_score:.2f}")
    ...
    async def test_05_resource_manager(self):
        ...
        print(f"✓ ResourceManager tested - Memory: {snapshot.memory_used_gb:.2f}GB / {manager.max_memory_gb}GB")
    ...
    async def test_06_async_pipeline(self):
        ...
        batch_results = await pipeline.process(
            batch_inputs,
            modality=Modality.TEXT,
            wait=True
        )
...
""")
    ...
    async def test_10_full_e2e_flow(self):
        ...
        for text, expected_fraud, result in results:
            ...
            print(f"  {status} '{text}...' - Score: {result.risk_assessment.overall_score:.2f}, "
