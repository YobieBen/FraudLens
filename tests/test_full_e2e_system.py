...
class TestFullE2ESystem:
    ...
    async def test_phishing_attack_detection_e2e(self, full_system):
        ...
        # Process text
        text_result = await full_system["pipeline"].process(
            phishing_email,
            modality="text"
        )
        ...
        # Process image
        image_result = await full_system["pipeline"].process(
            fake_website_img,
            modality="image"
        )
        ...
        pdf_result = await full_system["pipeline"].process(
            fake_invoice,
            modality="text"
        )
        ...
        assert risk_profile.risk_level in ["medium", "high", "critical"], "Should assess significant risk"
    ...
    async def test_money_laundering_detection_e2e(self, full_system):
        ...
        ml_matches = await full_system["pattern_matcher"].match_patterns(
            ml_data,
            tags=["money_laundering", "structuring", "behavioral"]
        )
        ...
        assert risk_profile.risk_level in ["medium", "high", "critical"], "ML should be significant risk"
    ...
    async def test_deepfake_fraud_detection_e2e(self, full_system):
        ...
        email_result = await full_system["pipeline"].process(
            ceo_fraud_email,
            modality="text"
        )
        print(f"   - Email fraud score: {email_result.fraud_score:.2f}")
        
        video_result = await full_system["pipeline"].process(
            deepfake_frame,
            modality="image"
        )
        ...
        assert risk_profile.risk_level in ["medium", "high", "critical"], "CEO fraud is significant risk"
    ...
    async def test_adaptive_learning_improvement_e2e(self, full_system):
        ...
        for i in range(10):
            ...
            baseline_cases.append({
                "case_id": f"BASELINE_{i:03d}",
                "result": result,
                "true_label": is_fraud
            })
        
        # Calculate baseline metrics
        baseline_tp = sum(1 for c in baseline_cases 
                          if c["true_label"] and c["result"].fraud_score > 0.5)
        baseline_fp = sum(1 for c in baseline_cases 
                          if not c["true_label"] and c["result"].fraud_score > 0.5)
        ...
        for i in range(10):
            ...
            improved_cases.append({
                "case_id": f"IMPROVED_{i:03d}",
                "result": result,
                "true_label": is_fraud
            })
        
        # Calculate improved metrics
        improved_tp = sum(1 for c in improved_cases 
                          if c["true_label"] and c["result"].fraud_score > 0.5)
        improved_fp = sum(1 for c in improved_cases 
                          if not c["true_label"] and c["result"].fraud_score > 0.5)
        
        improved_precision = improved_tp / (improved_tp + improved_fp) if (improved_tp + improved_fp) > 0 else 0
...
class TestSystemReliability:
    ...
    async def test_error_recovery_e2e(self, tmp_path):
        ...
        try:
            ...
            valid_results = sum(1 for r in results if not isinstance(r, Exception) and r is not None)
